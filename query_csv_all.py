...
if len(sys.argv) != 3:
    print('Usage: {0} http://localhost:9090 time(seconds)'.format(sys.argv[0]))
    sys.exit(1)
metrixNames=GetMetrixNames(sys.argv[1])
writeHeader=True
for metrixName in metrixNames:
      #now its hardcoded for hourly
      response = requests.get('{0}/api/v1/query'.format(sys.argv[1]), params={'query': metrixName+'[' + sys.argv[2] + 's]'})
